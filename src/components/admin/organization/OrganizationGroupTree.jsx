...
export default function OrganizationGroupTree({
  ...
  return (
    <section className="min-w-0 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm" aria-labelledby="organization-group-tree-title">
      <div className="flex flex-col items-stretch justify-between gap-3 sm:flex-row sm:items-center">
        <h3 id="organization-group-tree-title" className="text-xl font-bold text-gray-900">그룹 구조</h3>
        <button
          type="button"
          onClick={onAddRoot}
          className="min-h-12 w-full rounded-lg bg-green-700 px-4 py-2 font-bold text-white hover:bg-green-800 sm:w-auto"
        >
          최상위 조직 추가
        </button>
      </div>
          ...
          {flattened.map((group) => {
            ...
            return (
              <div
                ...
                className={`min-w-0 rounded-xl border p-3 ${group.id === selectedGroupId ? 'border-green-700 bg-green-50' : 'border-gray-200 bg-white'}`}
              ...
              >
                ...
                <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2 2xl:grid-cols-5">
                  <button type="button" onClick={() => onAddChild(group.id)} className="min-h-12 rounded-lg border border-green-700 px-2 font-semibold text-green-800" aria-label={`${group.name} 하위 조직 추가`}>하위 추가</button>
                  <button type="button" onClick={() => onMove(group.id, 'up')} disabled={siblingIndex <= 0} className="min-h-12 rounded-lg border border-gray-300 px-2 font-semibold disabled:cursor-not-allowed disabled:opacity-40" aria-label={`${group.name} 위로 이동`}>위로</button>
                  <button type="button" onClick={() => onMove(group.id, 'down')} disabled={siblingIndex === siblings.length - 1} className="min-h-12 rounded-lg border border-gray-300 px-2 font-semibold disabled:cursor-not-allowed disabled:opacity-40" aria-label={`${group.name} 아래로 이동`}>아래로</button>
