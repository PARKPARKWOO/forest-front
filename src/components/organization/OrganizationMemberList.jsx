  ...
  return (
    <ul aria-label={`${group.name} 구성원`} className="mt-6 grid min-w-0 gap-4 sm:grid-cols-2">
      {[...memberships].sort(byMembershipOrder).map((membership) => {
        ...
        const affiliation = resolveMembershipAffiliation(membership, person);
        return (
          <li key={membership.id} className="min-w-0 rounded-xl border border-green-100 bg-green-50/50 p-5">
            ...
            {membership.roleLabel && (
              <p className="mt-2 break-words text-lg font-semibold text-green-900">{membership.roleLabel}</p>
            )}
            {affiliation && (
