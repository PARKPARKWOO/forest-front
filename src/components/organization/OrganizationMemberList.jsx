...
export default function OrganizationMemberList({ group, peopleById, memberships }) {
  if (memberships.length === 0) {
    return (
      <p className="mt-6 rounded-xl bg-forest-surface-raised px-5 py-8 text-center text-lg text-forest-text-muted" role="status">
        등록된 공개 구성원이 없습니다
      </p>
    );
  }
  ...
  return (
    <ul aria-label={`${group.name} 구성원`} className="mt-6 grid min-w-0 gap-4">
      {[...memberships].sort(byMembershipOrder).map((membership) => {
        ...
        return (
          <li key={membership.id} className="min-w-0 rounded-xl border border-forest-border-subtle bg-forest-surface p-5">
            <h3 className="break-words text-xl font-bold text-forest-text-primary">{person.name}</h3>
            {roleLabel && (
              <p className="mt-2 break-words text-lg font-semibold text-forest-strong">{roleLabel}</p>
            )}
            {affiliation && (
              <p className="mt-2 break-words text-lg text-forest-text-muted">{affiliation}</p>
            )}
          </li>
        );
      })}
